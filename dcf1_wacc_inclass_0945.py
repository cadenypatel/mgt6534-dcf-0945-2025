# -*- coding: utf-8 -*-
"""DCF1 WACC - INCLASS 0945.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1h5rZOIJe3nnvVcHGPAe63yvIbxGMyib-
...
# Or get the last N observations as a time series
data = fred.get_series('GS10')

# Show the most recent yield
print(f"Latest 10-Year Treasury Yield: {data.iloc[-1]:.2f}% (as of {data.index[-1].date()})")
...
# Get the market cap and total debt from yfinance using the ticker object above
ticker_info = ticker.info
market_cap = ticker_info['marketCap'] / scale_factor
total_debt = ticker_info['totalDebt'] / scale_factor
...
def get_credit_spread(rating, credit_spreads):
    ...
    ----------
    rating : str
        The firm's credit rating (e.g., 'A3/A-', 'Baa2/BBB').
    credit_spreads : list of dict
        Lookup table of credit spreads (from Damodaran).

    Returns
    -------
    float
        The credit spread corresponding to the rating.
        If no match is found, prints a warning and returns NaN.
    """
...
# Example usage
# Example: use the lookup from the previously defined table
